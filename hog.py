...
def free_bacon(score):
    ...
    if score>0 :
        # 转化为字符串方便进行截取操作
        pi_str = str(pi)[1:]  #从小数点后一位开始截取，但是这里直接去除了小数点
        digit = int(pi_str[score-1])  #小数点后第score个数字，元组下标要-1
        return digit + 3
    pi=pi/10**(len(str(pi)-1))
    # END PROBLEM 2
    return pi % 10 + 3
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            who = 1
            if swine_align(score0, score1) or pig_pass(score0, score1):
                continue  # 如果触发特殊规则
            else:
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(current_score, last_score):
        nonlocal running_high
        if current_score > last_score:
            gain = current_score - last_score
            if gain > running_high:
                running_high = gain
                announcement = f"{gain} point(s)! The most yet for Player {who}"
                print(announcement)
        return say  # 递归
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def average_function(*args, **kwargs):
        total = 0
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_rolls = 0 
    # 遍历
    for num_rolls in range(1, 11):  # 骰子数量从1到10
        avg_score = make_averaged(lambda: roll_dice(num_rolls, dice), trials_count)()
        # 如果当前平均得分高于之前的最大平均得分，则更新
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
   # 检查是否触发额外回合
    if swine_align(score, opponent_score) or pig_pass(score, opponent_score):
    # 检查是否通过 Free Bacon 规则获得至少 cutoff 分数
        return bacon_strategy(score, opponent_score, cutoff, num_rolls)
    else:
        return num_rolls  # 否则，返回默认掷骰子数
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** 如果对手分数超过90,选择1次扔骰子，否则选择额外回合策略 ***
    """
    # BEGIN PROBLEM 12
    return 1 if opponent_score>=90 else extra_turn_strategy(score,opponent_score) # Replace this statement
    # END PROBLEM 12
